...
def create_section0(ventana):
    section0 = Frame(ventana,background=palette["bg_main"],relief="solid",borderwidth=5,name="section0")
    section0.pack()
    create_form(ventana)
...
def create_form(ventana):
    ...
    btn_save_income_outcome.grid(row=1,column=4,sticky="nsew")
def is_a_number(val):
    ...
    for i, row in enumerate(data):
        for j, item in enumerate(row):
            ...
            if j == 2 and es_numero(item) and float(item) > 0:
                ...
                label = Label(frame_grid, text=date_short_day(item) if j == 0 else item.replace(";",","),wraplength=obtener_resolucion_monitor_actual()[0]/3, anchor="w", justify="left", background=palette[grid_bg], font=palette["font_main"], fg=palette["font_color_main"])
                label.grid(row=i, column=j, padx=palette["pad_1"], pady=0, sticky="nsew")
...
def create_summary(ventana):
    ...
    lbl_summary_daily_type = Label(section2, text="Today's Summary",background=palette["bg_main"],fg=palette["font_color_main"],font=palette["font_1"],anchor="w", justify="left")
    ...
    lbl_summary_weekly_type = Label(section2, text="Last 7 days' Summary",background=palette["bg_main"],fg=palette["font_color_main"],font=palette["font_1"],anchor="w", justify="left")
    ...
    lbl_summary_monthly_type = Label(section2, text="Last 30 days' Summary",background=palette["bg_main"],fg=palette["font_color_main"],font=palette["font_1"],anchor="w", justify="left")
